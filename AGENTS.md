...
- **`docs/design-decisions.md`**: Chronological record of locked product and architectural decisions (navigation patterns, questions store & FNV-1a custom ID hashing, scoring curve semantics, multi-log continuous timeline scaling, sessionStorage check-in persistence, explicit naming standards) and open design questions. Consult this whenever verifying why a feature is designed a specific way or resolving architectural ambiguities.
