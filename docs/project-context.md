...
* **Dynamic Accessibility**: Theme switching (`dark`, `light`, `system`), contrast modes (`low`, `high`), configurable menu side (`right`, `left`), and an optional Hold-to-Confirm delay toggle (destructive/dismissing actions can require a 1.5s hold, or be disabled for instant click/tap).
